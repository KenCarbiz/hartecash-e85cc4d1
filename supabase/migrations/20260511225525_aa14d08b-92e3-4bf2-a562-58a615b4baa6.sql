...
CREATE POLICY "Service role full access purge_queue" ON public.customer_data_purge_queue
  FOR ALL TO service_role USING (true) WITH CHECK (true);

DROP POLICY IF EXISTS "Admins read own tenant purge_queue" ON public.customer_data_purge_queue;
CREATE POLICY "Admins read own tenant purge_queue" ON public.customer_data_purge_queue
...
DECLARE
  _req record;
  _sub_count int := 0; _call_count int := 0;
  _appt_count int := 0; _consent_count int := 0;
  _notif_count int := 0; _sub_ids uuid[];
BEGIN
  ...
  UPDATE submissions SET name = '[purged]', phone = NULL, email = NULL,
    vin = '[purged]', customer_memory = '[]'::jsonb,
    declined_reason = NULL, competitor_mentioned = NULL,
    customer_walk_away_number = NULL
  WHERE id = ANY(_sub_ids);
  GET DIAGNOSTICS _sub_count = ROW_COUNT;

  UPDATE voice_call_log SET customer_name = '[purged]', phone_number = '[purged]',
  ...
  UPDATE voice_call_turns SET text = '[purged]', pii_redacted_at = now()
  WHERE call_id IN (SELECT id FROM voice_call_log WHERE submission_id = ANY(_sub_ids));

  UPDATE voice_call_grades SET rationale = '[purged]', pii_redacted_at = now()
  WHERE call_id IN (SELECT id FROM voice_call_log WHERE submission_id = ANY(_sub_ids));

  UPDATE appointments SET customer_name = '[purged]', customer_email = '[purged]',
  ...
  UPDATE consent_log SET customer_name = '[purged]', customer_phone = NULL,
    customer_email = NULL, ip_address = NULL, user_agent = NULL
  WHERE (_req.contact_phone IS NOT NULL AND customer_phone = _req.contact_phone)
     OR (_req.contact_email IS NOT NULL AND customer_email = _req.contact_email);
  ...
  RETURN jsonb_build_object('ok', true,
    'submissions_purged', _sub_count, 'calls_purged', _call_count,
    'appointments_purged', _appt_count, 'consent_records_purged', _consent_count,
...
CREATE POLICY "Service role full access security_incident" ON public.security_incident
  FOR ALL TO service_role USING (true) WITH CHECK (true);

DROP POLICY IF EXISTS "Admins manage own tenant security_incident" ON public.security_incident;
CREATE POLICY "Admins manage own tenant security_incident" ON public.security_incident
...
BEGIN
  ...
  VALUES ('security_incident', 'fatal',
    'Security incident registered: ' || left(_summary, 240), _dealer::text,
