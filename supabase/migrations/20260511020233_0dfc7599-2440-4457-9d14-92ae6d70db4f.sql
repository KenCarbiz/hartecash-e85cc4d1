...
DECLARE
  ...
  allowed boolean;
BEGIN
  SELECT role::text, dealership_id, location_id, COALESCE(is_appraiser, false)
  INTO user_role, user_dealership, user_location, user_is_appraiser
  FROM public.user_roles
  WHERE user_id = _user_id
  ORDER BY CASE role::text
    WHEN 'admin' THEN 1 WHEN 'gsm_gm' THEN 2 WHEN 'gm' THEN 3
    WHEN 'used_car_manager' THEN 4 WHEN 'new_car_manager' THEN 5
    WHEN 'internet_manager' THEN 6 WHEN 'sales_bdc' THEN 7
    WHEN 'sales' THEN 8 WHEN 'receptionist' THEN 9 ELSE 99 END
  LIMIT 1;

  IF user_role IS NULL THEN RETURN '[]'::jsonb; END IF;

  IF user_role = 'admin' THEN
    SELECT COALESCE(jsonb_agg(s->>'key'), '[]'::jsonb) INTO result_keys
    FROM jsonb_array_elements(_sections) s;
    RETURN to_jsonb(result_keys);
  END IF;

  SELECT COALESCE(individual_sections, '{}') INTO individual
  FROM public.staff_permission_assignments
  WHERE user_id = _user_id AND permission_group_id IS NULL LIMIT 1;
  ...
  FOR section IN SELECT * FROM jsonb_array_elements(_sections) LOOP
    ...
    IF s_key = ANY(individual) THEN
      result_keys := array_append(result_keys, s_key); CONTINUE;
    END IF;
    ...
    IF allowed THEN result_keys := array_append(result_keys, s_key); END IF;
  END LOOP;
