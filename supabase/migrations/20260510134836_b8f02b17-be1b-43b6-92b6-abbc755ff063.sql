-- Idempotent: re-defines three existing RPCs with token_expiry guard.
-- Legacy rows where token_expires_at IS NULL continue to resolve.
...
RETURNS TABLE(
  ...
  self_checkin_at timestamp with time zone,
...
-- Ensure the helpers are granted
GRANT EXECUTE ON FUNCTION public.accept_offer(text) TO anon, authenticated;
GRANT EXECUTE ON FUNCTION public.mark_docs_uploaded(text) TO anon, authenticated;
