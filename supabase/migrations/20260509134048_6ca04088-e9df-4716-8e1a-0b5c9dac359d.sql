...
sliding AS (
  SELECT a.staff_user_id, a.staff_label, a.dealership_id,
...
CREATE POLICY "Service role full access posture snapshot"
  ON public.dealership_privacy_posture_snapshot FOR ALL TO service_role USING (true) WITH CHECK (true);
...
BEGIN
  ...
  INSERT INTO dealership_privacy_posture_snapshot
    (dealership_id, voice_transcript_retention_days, recording_url_retention_days,
     ...
     last_voice_redact_at, last_memory_redact_at,
     staff_pii_views_30d, distinct_staff_pii_viewers_30d,
     pending_data_requests, fulfilled_requests_90d,
     bulk_access_anomalies_14d, active_opt_outs,
     oldest_unredacted_call_at, retention_config_updated_at, snapshot_at)
  SELECT
    prc.dealership_id,
    ...
    (SELECT count(*) FROM v_bulk_access_anomalies WHERE dealership_id = prc.dealership_id),
    (SELECT count(*) FROM opt_outs WHERE dealership_id::text = prc.dealership_id),
...
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM cron.job WHERE jobname = 'refresh_dealership_privacy_posture') THEN
    PERFORM cron.unschedule('refresh_dealership_privacy_posture');
  END IF;
END $$;
