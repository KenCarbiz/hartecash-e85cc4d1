...
--
-- Trains the BDC team and feeds the AI voice agent's system prompt.
-- Each row is one objection with:
--   - the customer's actual words (signal we match against)
--   - their real underlying concern
--   - the dealer-voice reframe (what we say back)
--   - one or more proof points (concrete things we show)
--   - escalation path when the reframe doesn't close
--   - a voice_ai_snippet — a short, conversational version the
--     voice agent can read (NOT the email/SMS reframe; voice has
--     different cadence + must avoid lists, semicolons, jargon).
--   - which lead categories this fires on (decline_reason match,
--     competitor_mentioned match, hot_state, etc.)
--
-- Used by:
--   - The BDC rep's customer-file overlay (when declined_reason or
--     competitor_mentioned matches, the relevant card surfaces).
--   - The AI voice agent (compiled prompt block injected into the
--     Bland.ai system prompt at call-start via
--     get_objection_playbook_for_voice RPC).
--   - The Communications hub admin UI for in-place editing.
--
-- Per-tenant: dealers can override individual cards or add new ones
-- without affecting the network defaults. Public read so the unauthed
-- voice provider edge function can compile the prompt block.
--
...
CREATE TABLE IF NOT EXISTS public.objection_playbook (
  ...
  objection_key text NOT NULL,                 -- 'carmax_higher', 'private_sale_diy', etc.
  category text NOT NULL,                       -- 'price' | 'trust' | 'process' | 'competitor' | 'logistics' | 'timing'
  label text NOT NULL,                          -- "CarMax offered me more"
  customer_signals text[] NOT NULL DEFAULT '{}', -- phrases/keywords that match this objection
  customer_concern text NOT NULL,               -- the real underlying concern (BDC training)
  dealer_reframe text NOT NULL,                 -- what we say back (full prose)
  proof_points jsonb NOT NULL DEFAULT '[]'::jsonb, -- [{label, description}]
  escalation_path text,                          -- when reframe doesn't close
  voice_ai_snippet text,                         -- short conversational version for voice agent
  fires_on jsonb NOT NULL DEFAULT '{}'::jsonb,   -- {decline_reasons: [], competitors: []}
...
-- Seed the default catalog. Categories:
--   price       — number-based objections
--   trust       — privacy / safety / "who sees my info"
--   process     — how it works / friction
--   competitor  — named competitor or generic "they offered more"
--   logistics   — co-decider, payoff, title-not-in-hand, etc.
--   timing      — "not yet" / "thinking about it" / "next month"
...
-- RPC for the AI voice agent. Returns a compiled prompt block — one
-- objection per chunk — that gets injected into the voice provider's
-- system prompt at call-start. Voice agent reads voice_ai_snippet
-- specifically (the BDC-rep version is too prose-y for spoken
-- delivery).
