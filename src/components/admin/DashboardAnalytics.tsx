    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <div className="relative overflow-hidden bg-card rounded-xl border border-border p-4 shadow-sm">
          ...
          <div className="relative">
            <div className="flex items-center justify-between mb-1.5">
              <span className="text-micro text-muted-foreground font-bold uppercase tracking-wider">Total Leads</span>
            ...
            <p className="text-micro text-muted-foreground mt-0.5">{metrics.todayLeads} today / {metrics.weekLeads} this week</p>
          </div>
        </div>
        ...
        <div className="relative overflow-hidden bg-card rounded-xl border border-border p-4 shadow-sm">
          ...
          <div className="relative">
            <div className="flex items-center justify-between mb-1.5">
              <span className="text-micro text-muted-foreground font-bold uppercase tracking-wider">Avg Response</span>
              <Clock className="w-4 h-4 text-violet-500" />
            </div>
            <span className="text-3xl font-black text-card-foreground tracking-tight">{metrics.avgResponseDays}d</span>
            <p className="text-micro text-muted-foreground mt-0.5">Closed revenue: ${(metrics.closedDealValue / 1000).toFixed(0)}k</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="flex flex-col items-center gap-1 rounded-xl border border-border bg-card p-3 shadow-sm">
          ...
          <span className="text-micro text-muted-foreground font-medium">Hot Leads</span>
        </div>
        <div className="flex flex-col items-center gap-1 rounded-xl border border-border bg-card p-3 shadow-sm">
            ...
            return (
              <div key={src.key} className="flex items-center gap-3">
                ...
                <div className="flex-1 h-6 bg-muted/30 rounded-full overflow-hidden relative">
                  ...
                  >
                    {pct > 15 && <span className="text-micro font-bold text-white drop-shadow-sm">{src.count}</span>}
                  </div>
                </div>
        ...
        <div className="flex rounded-lg overflow-hidden h-8 bg-muted/30">
          {metrics.stageConversion.map((s, i) => {
            ...
            return (
              ...
              >
                {widthPct > 8 && (
                  <span className="text-micro font-bold text-white drop-shadow-sm">{s.count}</span>
                )}
              </div>
            );
          })}
        ...
        <div className="flex flex-wrap gap-x-4 gap-y-1 mt-2.5">
          {metrics.stageConversion.map((s, i) => (
            <div key={s.stage} className="flex items-center gap-1.5">
              ...
              <span className="text-micro text-muted-foreground">{STAGE_LABELS[s.stage]}: {s.count}</span>
            </div>
          ))}
        </div>
      </div>
