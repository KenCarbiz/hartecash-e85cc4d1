  ...
  const checkAll = async () => {
    ...
    const grouped: CheckGroup[] = [
      ...
      },
      {
        ...
        icon: Palette,
        items: [
          { key: "branding", label: "Dealership name configured", icon: FileText, done: !!(cfg?.dealership_name && cfg.dealership_name.trim().length > 0), section: "site-config:dealership-info" },
          { key: "logo", label: "Logo uploaded", icon: Image, done: !!(cfg?.logo_url && cfg.logo_url.length > 0), section: "site-config:logos" },
          { key: "logo_white", label: "White logo uploaded", icon: Image, done: !!(cfg?.logo_white_url && cfg.logo_white_url.length > 0), section: "site-config:logos" },
          { key: "favicon", label: "Favicon uploaded", icon: ScanLine, done: !!(cfg?.favicon_url && cfg.favicon_url.length > 0), section: "site-config:logos" },
          { key: "colors", label: "Brand colors set", icon: Palette, done: !!(cfg?.primary_color && cfg.primary_color !== "213 80% 20%"), section: "site-config:colors" },
        ],
      },
      {
