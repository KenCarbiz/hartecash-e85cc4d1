    ...
    <div className="space-y-8 max-w-5xl">
      ...
      <section className="bg-card rounded-xl border border-border p-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 mt-3">
          {LANDING_TEMPLATES.filter((t) => !["legacy", "moto", "clarity", "marquee", "velocity", "heritage"].includes(t.value)).map((t) => {
            const active = state.landing_template === t.value;
            return (
              <button
                key={t.value}
...
// ─── Value Tracker vehicle picker ──────────────────────────────
// Standalone admin card so the dealer can choose what vehicle the
// homepage Value Tracker section shows. Self-contained (own load,
// save, dirty state) so we don't have to touch the giant existing
// State machine above. Three modes:
//   oem     — auto-pick a flagship from the dealership name
//   popular — system-chosen mainstream vehicle (Toyota RAV4)
//   custom  — dealer-entered Year/Make/Model/Style
type TrackerMode = "oem" | "popular" | "custom";
...
const TRACKER_DEFAULTS: TrackerState = {
  mode: "oem", year: "", make: "", model: "", style: "",
};

const TrackerVehicleSection = () => {
  const { tenant } = useTenant();
  const dealershipId = tenant.dealership_id;
  const { toast } = useToast();
  const queryClient = useQueryClient();

  const [state, setState] = useState<TrackerState>(TRACKER_DEFAULTS);
  const [saved, setSaved] = useState<TrackerState>(TRACKER_DEFAULTS);
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  ...
  useEffect(() => {
    (async () => {
      ...
      const { data } = await supabase
        .from("site_config")
        .select("tracker_vehicle_mode, tracker_vehicle_year, tracker_vehicle_make, tracker_vehicle_model, tracker_vehicle_style")
        .eq("dealership_id", dealershipId)
        .maybeSingle();
      const next: TrackerState = {
        mode: ((data as any)?.tracker_vehicle_mode as TrackerMode) || "oem",
        year: (data as any)?.tracker_vehicle_year ? String((data as any).tracker_vehicle_year) : "",
        make: (data as any)?.tracker_vehicle_make ?? "",
        model: (data as any)?.tracker_vehicle_model ?? "",
        style: (data as any)?.tracker_vehicle_style ?? "",
      };
      setState(next);
      setSaved(next);
      setLoading(false);
  ...
  const dirty =
    ...
    state.make !== saved.make ||
    state.model !== saved.model ||
    state.style !== saved.style;

  const handleSave = async () => {
    setSaving(true);
    const year = state.year.trim() ? parseInt(state.year.trim(), 10) : null;
    const { error } = await supabase
      .from("site_config")
      .update({
        tracker_vehicle_mode: state.mode,
        tracker_vehicle_year: Number.isFinite(year as number) ? year : null,
        tracker_vehicle_make: state.make.trim() || null,
        tracker_vehicle_model: state.model.trim() || null,
        tracker_vehicle_style: state.style.trim() || null,
        updated_at: new Date().toISOString(),
      })
      .eq("dealership_id", dealershipId);
    setSaving(false);
    ...
    setSaved(state);
    queryClient.invalidateQueries({ queryKey: ["site_config"] });
    toast({ title: "Saved", description: "Tracker vehicle updated." });
  };

  const MODES: { value: TrackerMode; label: string; description: string }[] = [
    { value: "oem",     label: "Match my OEM brand",   description: "Auto-pick a flagship vehicle from your dealership name (Ford → Explorer, Infiniti → QX60)." },
    { value: "popular", label: "Use a popular vehicle", description: "Show a system-chosen mainstream crossover (Toyota RAV4) — best for independent / non-franchised stores." },
    { value: "custom",  label: "Pick my own vehicle",  description: "Enter the exact Year/Make/Model/Style to feature in the tracker card." },
  ];

  if (loading) {
    return (
      <section className="space-y-3">
        <h3 className="text-base font-bold">Value Tracker Vehicle</h3>
    ...
    <section className="space-y-4">
      <div>
        <h3 className="text-base font-bold flex items-center gap-2">
          <Layout className="w-4 h-4 text-primary" /> Value Tracker Vehicle
        </h3>
        <p className="text-sm text-muted-foreground">
          Controls the vehicle shown on the homepage "We track your vehicle value" section.
        </p>
      </div>
        ...
        {MODES.map((m) => {
          ...
          return (
            <button
              key={m.value}
              type="button"
              onClick={() => setState((s) => ({ ...s, mode: m.value }))}
      ...
      {state.mode === "custom" && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 p-4 rounded-lg border border-border bg-muted/30">
          <label className="text-xs font-medium space-y-1">
            <span>Year</span>
            <Input value={state.year} onChange={(e) => setState((s) => ({ ...s, year: e.target.value }))} placeholder="2022" inputMode="numeric" />
          </label>
          <label className="text-xs font-medium space-y-1">
            <span>Make</span>
            <Input value={state.make} onChange={(e) => setState((s) => ({ ...s, make: e.target.value }))} placeholder="Ford" />
          </label>
          <label className="text-xs font-medium space-y-1">
            <span>Model</span>
            <Input value={state.model} onChange={(e) => setState((s) => ({ ...s, model: e.target.value }))} placeholder="Explorer" />
          </label>
          <label className="text-xs font-medium space-y-1">
            <span>Style <span className="text-muted-foreground font-normal">(optional)</span></span>
            <Input value={state.style} onChange={(e) => setState((s) => ({ ...s, style: e.target.value }))} placeholder="XLT" />
          </label>
        </div>
      )}
